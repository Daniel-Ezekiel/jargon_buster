...
export async function POST(request: Request) {
  try {
    ...
    const topLabel = response.choices[0].message.content?.split(" ").slice(0, -1).join(" ");
    const confidence =
      (response.choices[0].message.content?.split(" ").at(-1) as unknown as number) * 100;

    return NextResponse.json({
      topLabel,
      confidence, // LLM discrete output
      source: "GPT-4o-mini",
    });
  } catch (err) {
