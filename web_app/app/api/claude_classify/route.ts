...
export async function POST(request: Request) {
  try {
    ...
    // console.log((response.content[0] as TextBlock).text);

    const topLabel = (response.content[0] as TextBlock).text.trim().split(" ").slice(0, -1).join(" ");
    const confidence = parseFloat((response.content[0] as TextBlock).text.trim().split(" ").at(-1) || "0") * 100;
